 ...
 * @throws {@link PublicError.Widgets.UnknownError} 알 수 없는 오류가 발생한 경우
 */
type RequestPayment$2 = RequestPaymentWithRedirection$1 & RequestPaymentWithPromise$1;
/**
 ...
 * @throws {@link PublicError.Widgets.BelowZeroAmountError} 결제 금액이 0원 이하인 경우
 * @throws {@link PublicError.Widgets.InvalidParametersError} 파라미터가 올바르지 않은 경우
 * @throws {@link PublicError.Widgets.UnknownError} 알 수 없는 오류가 발생한 경우
 */
 ...
 * @throws {@link PublicError.Brandpay.BelowZeroAmountError} 결제 금액이 0 미만인 경우
 ...
 */
type RequestPayment$1 = RequestPaymentWithRedirection & RequestPaymentWithPromise;
/**
 * @docsAlias Redirect 방식
...
interface PaymentRequest {
    ...
    /**
     * 결제창이 열리는 프레임입니다.
     * `iframe` 또는 현재 브라우저 화면(`self`)에서 결제창을 열 수 있어요. 기본 값은 `iframe`입니다.
     * 모바일 웹에서는 `windowTarget` 값이 작동하지 않고 화면이 이동합니다.
     */
...
interface BillingAuthRequest {
    ...
    /**
     * 결제창이 열리는 프레임입니다.
     * `iframe` 또는 현재 브라우저 화면(`self`)에서 등록창을 열 수 있어요. 기본 값은 `iframe`입니다.
     * 모바일 웹에서는 `windowTarget` 값이 작동하지 않고 화면이 이동합니다.
     *
     * @default 'iframe'
     */
 ...
 * @throws {@link PublicError.Payment.BelowZeroAmountError} 결제 금액이 0 미만인 경우
...
/**
 * @docsAlias 해외 간편결제(Promise 방식)
 * @returns `WidgetPaymentResult` 객체가 응답됩니다. 객체 필드를 확인하고 [결제 승인 API](/reference#결제-승인)를 호출해야 결제가 최종적으로 완료돼요.
 * @param {RequestForeignEasyPayPaymentWithPromise} paymentRequest 결제 요청 정보입니다.
 *
 * @example
 *  ```javascript
 *  payment.requestPayment({
 ...
 * ```
 */
type RequestForeignEasyPayPaymentWithPromise = (paymentRequest: ForeignEasyPayPaymentRequest) => Promise<RequestPaymentResult>;
/**
...
interface TossPaymentsSDK {
     ...
     * @throw {@link PublicError.Widgets.WidgetsSDKInitializeConflictError} widgets SDK 초기화 중 충돌이 발생한 경우
     ...
     * @throw {@link PublicError.Brandpay.BrandpaySDKInitializeConflictError} 브랜드페이 SDK 초기화 중 충돌이 발생한 경우
     ...
     * @throw {@link PublicError.Payment.PaymentSDKInitializeConflictError} payment SDK 초기화 중 충돌이 발생한 경우
     * @throw {@link PublicError.Brandpay.UnknownError} 알 수 없는 오류가 발생한 경우
     * @example
     * ```javascript
 ...
 *
 * 결제위젯을 연동한다면 [결제위젯 연동 키](/reference/using-api/api-keys#결제위젯-연동-키)를 사용하세요. 결제창, 자동결제(빌링)을 연동한다면 [API 개별 연동 키](/reference/using-api/api-keys#api-개별-연동-키)를 사용하세요.
 *
 * @returns
